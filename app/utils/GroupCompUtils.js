...
class Comp1Utils {
    ...
    const data = await actionOptionIds.forEach (async actionOptionId => {
      ...
      userMetricIds.concat(userEventMetricIds);
        ...
        if (vals.length) {
            ...
            return await this.saveAI(uliId, sprintNumber, actionOptionId ,ai,transaction);
        }
  ...
  async saveAI(uliId, sprintNumber, actionOptionId ,value,transaction){
    console.log('saving-------------------')
    console.log("action ",actionOptionId," value",value)
    console.log('saved----------------------')
    return await models.user_action_option_impact_sprint_values.create({
        uliId,
        sprintNumber,
        actionOptionId,
        value
